...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源
        selectedAPIs = ["dyttzy", "ruyi", "bfzy", "tyyszy", "xiaomaomi", "ffzy", "heimuer", "zy360", "iqiyi", "wolong", "hwba", "jisu", "dbzy", "mozhua", "mdzy", "zuid", "yinghua", "baidu", "wujin", "wwzy", "ikun",  "lzi", "qiqi", "maotai"];
...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `小尨TV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== '小尨TV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = '小尨TV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), '小尨TV-Settings_' + times + '.json');
}
